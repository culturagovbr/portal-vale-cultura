...
class Minc_IndexController extends GenericController
{
    ...
    public function listaBeneficiariaAction()
    {
        ...
        if ($_POST) {
            ...
            if (is_array($DTINICIO)) {
                if (!checkdate($DTINICIO[1], $DTINICIO[0], $DTINICIO[2])) {
                    parent::message('Data de Cadastro (M�nima) inv�lida.', '/minc/admin/lista-beneficiarios/', 'error');
                }
            }

            if (is_array($DTFIM)) {
                if (!checkdate($DTFIM[1], $DTFIM[0], $DTFIM[2])) {
                    parent::message('Data de Cadastro (M�xima) inv�lida.', '/minc/admin/lista-beneficiarios/', 'error');
                }
            }
